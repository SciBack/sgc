...
- El árbol del marco (Elemento Marco, is_tree) da estándares (tipo=Estandar) y sus criterios
  (hijos por `parent_elemento_marco`). Reusamos los helpers de `sgc.scoring`.
- Evidencias por estándar: `Trazabilidad` (evidencia + elemento_marco), sobre el estándar
  y sobre cada uno de sus criterios.
...
def _criterios_consolidados(autoevaluacion, estandar_name):
    """Lista de dicts por criterio del estándar: ref_id, denominación, cumple, observación."""
    filas = []
    for crit in _criterios_de_estandar(estandar_name):
        meta = frappe.db.get_value(
            "Elemento Marco", crit, ["codigo", "denominacion", "texto_oficial"], as_dict=True
        ) or {}
...
def consolidar(autoevaluacion):
    ...
    resumen = []
    todas_evid = {}
    for est in _estandares_de_autoevaluacion(autoevaluacion):
        meta = frappe.db.get_value(
            "Elemento Marco", est, ["codigo", "denominacion", "texto_oficial"], as_dict=True
        ) or {}
        ...
        criterios = _criterios_consolidados(autoevaluacion, est)
        elementos_evid = {est} | set(_criterios_de_estandar(est))
